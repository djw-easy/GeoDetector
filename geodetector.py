...
def interaction_detector(df, y, factors):
    ...
    for i in range(0, length):
        for j in range(0, i+1):
            ssw = df[[y]+list(set([factors[i], factors[j]]))].groupby([factors[i], factors[j]]).apply(
                lambda x:x.shape[0]*x.var(ddof=0))[y].sum()
            sst = (df[y].size*df[y].var(ddof=0))
            q = 1-ssw/sst
...
def ecological_detector(df, y, factors):
    ...
    for i in range(1, length):
        ssw1 = df[[y, factors[i]]].groupby(factors[i]).apply(lambda x:x.shape[0]*x.var(ddof=0))[y].sum()
